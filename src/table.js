...
module.exports = React.createClass({
  ...
  handleHeadingClick: function (data) {
    const activeKey = this.state.activeSortKey;
  ...
  filterObject: function (obj) {
    let filteredData;
    const includedColumns = this.props.includedColumns;
  ...
  renderHead: function () {
    const columns = this.generateHeadersFromRow(this.props.data[0]);
    return (
      <TableHead
        columns={columns}
  ...
  sortRow: function (options, rowA, rowB) {
    let a = rowA[options.key];
    let b = rowB[options.key];
