describe('Table', function () {
  ...
  function trsContain(table, array) {
    const trs = selecTrs(table);
    _.zip(trs, array)
      .forEach(function (pair) {
        const actual = parseInt(pair[0].getDOMNode().textContent);
  ...
  it('uses an optional array to filter which properties show up in rows', function () {
    ...
    const row = data[0];
    const columns = ['include', 'alsoInclude'];
  ...
  describe('#renderHeader', function () {
    it('defaults to creating headers based on key names of data', function () {
      const table = render(<Table data={fixtures.data}/>);
      const headers = table.generateHeadersFromRow(fixtures.data[0]);
  ...
  describe('sorting', function () {
    ...
    it('takes an optional default sort parameter', function () {
      const table = render(<Table data={sortData} initialSortKey='name' />);
      const trs = selecTrs(table);
    ...
    it('properly sorts numerical items', function () {
      const key = 'id';
      const numericalData = [15, 47, 7, 7, 12, 15, 7, 15, 15, 27, 47].map(function (x) { return {id: x}; });
      const sorted = [7, 7, 7, 12, 15, 15, 15, 15, 27, 47, 47];

      const table = render(<Table data={numericalData} />);
    ...
    it('properly sorts alphabetical items', function () {
      const key = 'id';
      const data = ["SUFFOLK", "NASSAU", "SUFFOLK", "WESTCHESTER", "WESTCHESTER", "ONONDAGA", "WESTCHESTER", "WESTCHESTER", "SUFFOLK", "ONONDAGA", "ONONDAGA", "ONONDAGA", "WESTCHESTER", "WESTCHESTER", "SUFFOLK", "ONONDAGA", "ONONDAGA", "ONONDAGA", "ONONDAGA", "SUFFOLK", "SUFFOLK", "SUFFOLK", "SUFFOLK", "SUFFOLK", "WESTCHESTER"].map(function (x){ return {name: x}; });

      const sorted = ["NASSAU", "ONONDAGA", "ONONDAGA", "ONONDAGA", "ONONDAGA", "ONONDAGA", "ONONDAGA", "ONONDAGA", "ONONDAGA", "SUFFOLK", "SUFFOLK", "SUFFOLK", "SUFFOLK", "SUFFOLK", "SUFFOLK", "SUFFOLK", "SUFFOLK", "SUFFOLK", "WESTCHESTER", "WESTCHESTER", "WESTCHESTER", "WESTCHESTER", "WESTCHESTER", "WESTCHESTER", "WESTCHESTER"];

      const table = render(<Table />);
      const actual = table.sortRows(data).map(function (x) { return x.name; });
    ...
    it('sorts rows in ascending order by default, using the 1st key of a row as comparator', function () {
      const table = render(<Table data={sortData} />);
      const trs = selecTrs(table);

      const smallestId = sortData[0].id + sortData[0].name;
      const largestId = sortData[1].id + sortData[1].name;
    ...
    it('sorts with a key provided by state.sortKey', function () {
      ...
      const table = render(<Table data={data} />);
  ...
  describe('#renderHead', function () {
    it('passes columnDisplay to head', function () {
      ...
      const header = TestUtils.findRenderedComponentWithType(table, TableHead);
      const mappedHeaders = Object.keys(header.props.columnDisplay);
